...
def criar_grafico(
    tipo, dados, titulo, cores=px.colors.qualitative.Set2, hole=0.4
):
    """
...
def grafico_comparativo(
    ...
    titulo='Comparação de Distribuições',
):
    """
    ...
    )  # Escolhe uma paleta qualitativa do Plotly
    ...
    fig = go.Figure(
        data=[
            ...
            go.Bar(
                ...
                marker=dict(
                    color=cores[1]
                ),  # Cor para a distribuição desejada
            ),
        ]
    )
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            title='Ativos', tickfont=dict(size=14)
        ),  # Ajusta a fonte dos rótulos do eixo X
        yaxis=dict(
            title='Valor Investido (R$)', tickfont=dict(size=14)
        ),  # Ajusta a fonte dos rótulos do eixo Y
        title_font=dict(size=24, family='Arial, sans-serif', color='black'),
